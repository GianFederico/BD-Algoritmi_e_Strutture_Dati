...
typename List_vector< T >::position List_vector< T >::next(position p) const {
	if ( (p > 0) && (p < length_+1)) // precondizione
		return(p+1);
	else
		return(p);
}
...
typename List_vector< T >::position List_vector< T >::previous(position p) const {
	if ( (p > 1) && (p < length_+1)) // precondizione
		return(p-1);
	else
		return(p);
}
...
void List_vector< T >::insert(const value_type &a, position p){
  if (length_ == array_dimension_){
    change_dimension_(elements_, array_dimension_, array_dimension_ +(array_dimension_/2));
...
template<class T>
void List_vector<T>::push_front(const value_type &a){
    insert(a,begin());
}

template<class T>
void List_vector<T>::push_back(const value_type &a){
    position p;
    p=length_+1;
