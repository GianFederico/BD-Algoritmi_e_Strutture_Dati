...
private:
 ...
 Hash<K> hashm;		            // maps type K to nonnegative integer   f(k)=k%D --> f(k)=(the_key) % divisor
 int dsize;		                // number of entries in dictionary [num. elementi nella tabella]
 int divisor;		            // hash function divisor [dimensione max tabella]
};
 ...
 * 1) table[b] points to an entry whose key is the_key   [chiave e valore esistono]
 * 2) no entry in the table has the key the_key, table[b] is NULL   [-chiave esiste, valore no]
 * 3) no entry in the table has the the_key, table[b] has a key other than the_key, and the   [-chiave non esiste]
 *   table is full
 */
...
int HashTable<K,E>::search(const K& the_key) const
{
  ...
  } while (j != i);                         //controlla tutte le posizioni nella tabella fino a ritornare a i
...
Entry< K, E>* HashTable<K,E>::find(const K& the_key) const
{
  ...
  return table[b];  // matching entry(bucket)
}
